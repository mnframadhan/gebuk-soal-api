...
app.use(
    cors({
        ...
        allowedHeaders: "Content-Type, X-API-TOKEN, X-API-TOKEN-STUDENT, X-API-TOKEN-COMPANY, X-API-TOKEN-ADMIN, PACKAGE-BUNDLE-TOKEN",
    })
);
...
app.get("/", (req, res) => {
    res.status(200);
    res.send("Hello, This is Ruang Ujian API");
});
...
app.get("/api/complete-package", CompletePackageControllers.getAllCompletePackages);
