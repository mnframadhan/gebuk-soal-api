...
export class CandidateController {
    ...
    static async candidateEmailVerification(req: StudentReq, res: Response, next: NextFunction) {
        try {
            const request: { verification_code: string } = req.body;
            const response = await CandidateService.candidateEmailVerification(request, req.student!);
    ...
    static async checkPackageBundleToken(req: StudentReq, res: Response, next: NextFunction) {
        try {
            ...
            if (response.token) {
                res.cookie("PACKAGE-BUNDLE-TOKEN", response.token, { maxAge: 1000 * 60 * 60 * 24, httpOnly: false });
            }
