import { ContributorLoginRequest, ContributorRequest, ContributorResponse, minimizedSoalCreatedResponse, SmallContributorResponse, toContributorLeaderboard, toContributorResponse, toMinimizedSoalCreatedResponse } from "../model/contributor-model";
...
export class ContributoService {
    ...
    static async getSoalCreated(page: number, limit: number, contributor: Contributor) : Promise<Pageable<minimizedSoalCreatedResponse>> {

        const skip = (page - 1) * limit;
        const pagination: Paging = {

            size: limit,
            total_page: Math.ceil(Number(await prismaClient.soal.count({ where: { created_by: contributor.username } })) / limit),
            current_page: page
